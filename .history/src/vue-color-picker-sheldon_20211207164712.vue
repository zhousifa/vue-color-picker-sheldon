<template>
  <div :class="['ColorPicker',draggable?'draggable':'']" id="color-picker">
    ...
    <div class="block stable">
      <input class="slider" v-model="opcity" id="slider" type="range" :min="0" :max="1" :step="0.01"  />
    </div>
    <!-- 选中颜色展示区域 -->
    ...
    <div class="colorType">
      ...
      <!-- 手写switch标签 -->
      <div class="switch">
        <span :class="['switch-label','switch-label-left',colorType?'':'active-label']">hex</span>
        <span :class="['switch-core',colorType?'active-switch-core':'']" @click="changeColorType"></span>
        <span :class="['switch-label','switch-label-right',colorType?'active-label':'']">rgba</span>
      </div>
    </div>
    
    <!-- hex颜色输入框区域 -->
    <input v-show="!colorType" class="colorValue stable" v-model="colorValue.hex" maxlength="9" placeholder="请选择或者输入颜色"/>
    <!-- rgba颜色输入框区域 -->
    <div v-show="colorType" class="rgbaValue stable">
      <div class="col" v-for="(item,index) in colorValue.rgb" :key="index">
        <input maxlength="3" v-model="colorValue.rgb[index]" max="9">
      </div>
      <div class="col">
        <input v-model="rgbOpcity"/>
      </div>
    </div>
    <!-- 按钮区域 -->
    <div>
      <button class="stable" @click="sendColor" type="primary">确定</button>
    </div>
  </div>
</template>
    ...
    sendColor(){
      ...
      if(!this.colorType){
        ...
        if(len === 4 || len === 5 || len === 7 || len === 9){
          ...
          return alert('当前颜色值不符合格式，请修改后进行切换')
        }
      }
      // 发送rgb
      else{
